...
router.get("/game-scores", async (req, res) => {
  try {
    ...
    await errorLogService.logRouteError(error, 'gameScoreRoutes.js', { route: '/game-scores', method: 'GET' });
...
router.get("/game-scores/:id", async (req, res) => {
  try {
    ...
    await errorLogService.logRouteError(error, 'gameScoreRoutes.js', { route: '/game-scores/:id', method: 'GET', id });
    res.status(400).json({ error: error.message });
  }
});
...
router.get("/games/:gameId/game-scores", async (req, res) => {
  try {
    ...
    await errorLogService.logRouteError(error, 'gameScoreRoutes.js', { route: '/games/:gameId/game-scores', method: 'GET', gameId });
    res.status(400).json({ error: error.message });
  }
});
